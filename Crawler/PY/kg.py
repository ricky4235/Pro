...
X = pd.DataFrame({'city':['tokyo',None,'london','seattle','san fancisco','tokyo'], 
                  ...
                  'quantitative_column':[1,11,-.5,10,None,20]}) X

首先我們需要對分類變量進行填充操作，類別變量一般用眾數或者特殊值來填充，
回顧之前的內容，我們也還是採取Pipeline的方式來進行，因此可以事先基於TransformMixin
基類來對填充的方法進行封裝，然後直接在Pipeline中進行調用，代碼可以參考：
